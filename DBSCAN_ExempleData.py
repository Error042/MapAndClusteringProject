...
-- eps :: in radius 0.02
'''
model = DBSCAN(eps=0.11, min_samples=20, metric='euclidean').fit(dbscan_data)
print(model)
...
fig, ax = plt.subplots()
ax.scatter(clusters_df['x'], clusters_df['y'], c=colors_clusters, edgecolors='black', s=50)
ax.scatter(outliers_df['x'], outliers_df['y'], c=color_outliers, edgecolors='black', s=5)
ax.set_xlabel('X', family='Arial', fontsize=9)
ax.set_ylabel('Y', family='Arial', fontsize=9)
